...
if __name__ == "__main__":
    ...
    #IMPROVEMENT: What you can do here is to not download the full dataset, instead get and process from S3 bucket.
    full_dataset = load_dataset(args.hf_dataset_name, "distractor", split="train", trust_remote_code=True)
    new_full_dataset = full_dataset.map(template_dataset, remove_columns=list(full_dataset.features))
    full_input_path = "/opt/ml/processing/full"
    new_full_dataset.save_to_disk(full_input_path)    
